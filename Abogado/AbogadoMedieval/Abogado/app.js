...
let orquitoestado = 0; // Estado del orquito
...
function comprobarLetra(letra) {
    ...
    if (palabraAleatoria.includes(letra)) {
        ...
        // Si la letra no está, cambiar la imagen del orquito
...
function cambiarImagen() {
    const imgOrquito = document.getElementById("orquito");
    // Cuando fallamos, la imagen se incrementa
    orquitoestado++;
    // Cambiamos la imagen aprovechando el aumento anterior
    imgOrquito.src = "images/orquito-" + orquitoestado + ".png";
    // Si el estado es mayor o igual al maximo, mostramos el mensaje de game over
    if (orquitoestado >= error_maximo) { // el estado de orquito llega al error máximo
        mostrarGameOver();
...
function resetearJuego() {
    orquitoestado = 0; // reseteamos el estado del orquito
    ...
    const imgOrquito = document.getElementById("orquito");
    imgOrquito.src = "images/orquito-0.png"; // reseteamos la imagen del orquito
